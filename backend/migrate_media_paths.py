...
def run_migration():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute("PRAGMA table_info(movies)")
        ...
        for old_name, new_name in renames.items():
            if old_name in columns and new_name not in columns:
                ...
                conn.execute(f"ALTER TABLE movies RENAME COLUMN {old_name} TO {new_name}")
        ...
        if "video_original_filename" not in columns_updated:
            ...
            conn.execute("ALTER TABLE movies ADD COLUMN video_original_filename VARCHAR(255)")
        ...
        try:
            ...
            for col in ["poster_path", "backdrop_path", "video_source_path", "hls_playlist_path"]:
                if col in columns_updated or col in renames.values():
                    conn.execute(f"UPDATE movies SET {col} = REPLACE({col}, 'http://localhost:8000/uploads/', 'media/') WHERE {col} LIKE 'http://%'")
